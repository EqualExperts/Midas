...
echo "2. starting version v1 of clientApp connected to mongo on port 27017"
start "client-v1" groovy -cp libs\mongo-java-driver-2.11.3.jar; Client.groovy --version=v1 --host=localhost --port=27020

timeout 20000

echo "3. throw in deltas for version v2"
start groovy DeltaGenerator.groovy --version=v2

timeout 10

echo "4. stop v1 client"
taskkill /F /T /FI "WINDOWTITLE eq client-v1*"

echo "5. starting version v2 of clientApp connected to Midas on port 27020"
