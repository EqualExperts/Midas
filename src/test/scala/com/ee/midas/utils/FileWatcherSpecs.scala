...
class FileWatcherSpecs extends Specification {
  ...
  "File Watcher" should {
    ...
    "perform given action once during initialization" in {
      ...
      //when: A FileWatcher is initialized
        new FileWatcher(file.getAbsolutePath, 1, TimeUnit.SECONDS)(watchAction)
    ...
    "perform given action whenever the file is modified" in {
      ...
      //and: a FileWatcher running on it
        val watcher = new FileWatcher(file.getAbsolutePath, 5, TimeUnit.MILLISECONDS)(watchAction)
        watcher.start()
